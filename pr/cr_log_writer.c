...
main(void)
{
    ...
    FILE *f  = fdopen(fd, "w");
    // writer_t *writer = writer_create(fd, 8 * 1024L);
    writer_t *writer = writer_create(fd, 8 * 1024L);

    clock_gettime(CLOCK_MONOTONIC, &start);
    for (i32 i = 2; i < RUNS; i++) {
        // writer_write(writer, message, 13);
        // printf("%s", message);

        // 42ns
        // 1.06s user 0.86s system 44% cpu 4.279 total
        // printf("%d\n", i);
        // writer_write_i64(writer, i);

        // 60ns
        // 5.23s user 0.81s system 99% cpu 6.068 total
        // printf("%ld", 123L);
        (void)fprintf(f, "%d", i);
    }
    ...
    i64 time_taken
        = ((i64)(end.tv_sec * 1000000000LL) + end.tv_nsec) - ((i64)(start.tv_sec * 1000000000LL) + start.tv_nsec);
